...
public class FundComputingJob {
	...
	public void fundComputing() throws Exception{
		...
		for(DrtFinPoAccountRecord drtFinPoAccountRecordTemp:lstDrtFinPoAccountRecord){
			...
			if(drtFinPoAccountRecordTemp.getIsInOut().equals("1")){
		...
		DrtMonitorFundDay drtMonitorFundDayQuery = new DrtMonitorFundDay();
		drtMonitorFundDayQuery.setPresentDate(drtMonitorFundDay.getPresentDate());
		List<DrtMonitorFundDay>	drtMonitorFundDayLst = drtMonitorFundDayService.selectList(drtMonitorFundDayQuery);
		if(drtMonitorFundDayLst.size() > 0){
			drtMonitorFundDay.setId(drtMonitorFundDayLst.get(0).getId());
			drtMonitorFundDayService.update(drtMonitorFundDay);
		}else{
