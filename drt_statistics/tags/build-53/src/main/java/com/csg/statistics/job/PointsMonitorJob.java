...
public class PointsMonitorJob {
    ...
    @Autowired
    private DrtOprPointsMonitoringService drtOprPointsMonitoringService;
     ...
     */
    // @Scheduled(cron = "10 * * * * ?")
    public void pointsTypesTatistics() throws Exception {
        ...
        // 获取积分来源为收益的积分信息
        Map<String, Object> proceedsEarningsMap = drtOprPointsMonitoringService.queryEarnings(todayTime, yesterdayTime,
            weekAgoTime, SystemConstants.EARNINGS_TYPE_PROCEEDS);
        // 获取积分来源为推广的积分信息SystemConstants.EARNINGS_TYPE_SPREAD为推广
        Map<String, Object> spreadEarningsMap = drtOprPointsMonitoringService.queryEarnings(todayTime, yesterdayTime,
            weekAgoTime, SystemConstants.EARNINGS_TYPE_SPREAD);
        // 获取积分来源为消费的积分信息SystemConstants.EARNINGS_TYPE_CONSUME为推广
        Map<String, Object> paymentEarningsMap = drtOprPointsMonitoringService.queryEarnings(todayTime, yesterdayTime,
            weekAgoTime, SystemConstants.EARNINGS_TYPE_CONSUME);
        
        // 消费积分
        Map<String, Object> consumeEarningsMap = drtOprPointsMonitoringService.queryConsumeEarnings(todayTime,
            yesterdayTime, weekAgoTime);
        // 剩余积分的分布情况
        Map<String, Object> remainEarningsMap = drtOprPointsMonitoringService.queryRemainEarnings(todayTime);
        // 封装数据
        ...
        DrtAccount drtAccount = new DrtAccount();
        // accountStatus-0:正常
        drtAccount.setAccountStatus("0");
        long accountNum = PageHelper.count(() -> drtAccountService.selectList(drtAccount));
        monitorPointsDay.setPresentAccountNum(new Long(accountNum).intValue());
        // 查询是否有数据，如果有，则更新，如果没有，插入
        DrtMonitorPointsDay drtMonitorPointsDay = new DrtMonitorPointsDay();
        drtMonitorPointsDay.setPresentDate(Integer.valueOf(yesterday));
        long pointNum = PageHelper.count(() -> drtOprPointsMonitoringService.selectList(drtMonitorPointsDay));
        // 插入到积分监控表中
        if (pointNum > 0) {
            drtOprPointsMonitoringService.update(monitorPointsDay);
        } else {
            drtOprPointsMonitoringService.insert(monitorPointsDay);
        }
    }
