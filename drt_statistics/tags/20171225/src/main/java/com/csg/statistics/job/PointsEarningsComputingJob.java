...
public class PointsEarningsComputingJob {
								...
								if (points > 0) {
									...
									drtFinPoEarningsRecord
											.setTime(new Timestamp(System
													.currentTimeMillis()));
									// 插入记录并更新积分
									drtFinPoEarningsRecordService
											.insertAndUpdatePoints(drtFinPoEarningsRecord);
								}
							}
						}
					}
	 ...
	 * 
	 */
	public void pointsEarningsMoneyComputing() {
		try {
			// 预购账户积分收益计算明细汇总
			...
			objQueryDrtFinPoEarningsComputingRecord
					.setEarningsComputingTime(NumberUtils
			...
			List<DrtFinPoEarningsComputingRecord> lstDrtFinPoEarningsComputingRecord = drtFinPoEarningsComputingRecordService
					.selectList(objQueryDrtFinPoEarningsComputingRecord);
	 ...
	 *
	 */
	public void pointsEarningsTimePoint(){
		try{
			...
			List<DrtFinPoAccountRecord> finPoAccountRecordList = drtFinPoAccountRecordService.selectListYesterday(drtFinPoAccountRecordForQuery);
			...
			for (DrtFinPoAccountRecord drtFinPoAccountRecord : finPoAccountRecordList) {
				if(drtFinPoAccountRecord.getPoDealMoney().longValue() < 0){
					drtFinPoEarningsComputingRecordService.moneyConsume(drtFinPoAccountRecord.getPoAccountId(), drtFinPoAccountRecord.getPoDealMoney().longValue());
				}else{
