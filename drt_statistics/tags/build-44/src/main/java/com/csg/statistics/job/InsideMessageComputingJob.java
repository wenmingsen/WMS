...
public class InsideMessageComputingJob {
	...
	public void insertTimingNewsComputingJob() {
		...
		try {
			...
			// 站内消息list
			List<DrtOprInsideMessage> lstInsertInsideMessage = new ArrayList<DrtOprInsideMessage>();
			if (!CollectionUtils.isEmpty(lstDrtOprInsideMessage)) {
				...
				for (DrtOprInsideMessage objInsideMessage : lstDrtOprInsideMessage) {
					...
					if(iSendTime <= iPresentDate){
						...
						// 更新站内消息状态
						lstInsertInsideMessage.add(objInsideMessage);
			...
			if(!CollectionUtils.isEmpty(lstInsertInsideMessage)){
				//插入站内消息
				drtOprInsideMessageService.updateInsideMessages(lstInsertInsideMessage);
			}
		} catch (Exception e) {
			...
			if (!CollectionUtils.isEmpty(lstDrtOprInsideMessage)) {
				...
				for (DrtOprInsideMessage objInsideMessage : lstDrtOprInsideMessage) {
					...
					if(iPresentDate > iEndTime){
						...
						objInsideMessage.setMessageState(2);
						lstInsertInsideMessage.add(objInsideMessage);
					}else if(iPresentDate <= iEndTime && iPresentDate>= iStartTime){
						//站内消息状态(0.未发送1.发送中2.已发送)
						objInsideMessage.setMessageState(1);
						lstInsertInsideMessage.add(objInsideMessage);
					}
