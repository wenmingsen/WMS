...
public class InsideMessageComputingJob {
	...
	public void insertTimingNewsComputingJob() {
		...
		// 流程状态(0.待上报1.审批中2.审批完成3.不通过)
		objInsideMessageQuery.setAuditState(2);
		// 发送类型（1-定时 2- 循环）
		...
		try {
			...
			if (!CollectionUtils.isEmpty(lstDrtOprInsideMessage)) {
				...
				for (DrtOprInsideMessage objInsideMessage : lstDrtOprInsideMessage) {
					...
					if(iSendTime <= iPresentDate && objInsideMessage.getMessageState() !=null 
							&& objInsideMessage.getMessageState() != 2){
						//站内消息状态(0.未发送1.发送中2.已发送)
						objInsideMessage.setMessageState(2);
						drtOprInsideMessageService.update(objInsideMessage);
					}else if(iSendTime > iPresentDate && objInsideMessage.getMessageState() !=null 
							&& objInsideMessage.getMessageState() != 1){
						//站内消息状态(0.未发送1.发送中2.已发送)
						objInsideMessage.setMessageState(1);
						// 更新站内消息状态
						drtOprInsideMessageService.update(objInsideMessage);
					}
		...
		try {
			...
			if (!CollectionUtils.isEmpty(lstDrtOprInsideMessage)) {
				...
				for (DrtOprInsideMessage objInsideMessage : lstDrtOprInsideMessage) {
					...
					if(iPresentDate > iEndTime  && objInsideMessage.getMessageState() !=null 
							&& objInsideMessage.getMessageState() != 2){
						//站内消息状态(0.未发送1.发送中2.已发送)
						objInsideMessage.setMessageState(2);
						//更新站内消息
						drtOprInsideMessageService.update(objInsideMessage);
					}else if(iPresentDate <= iEndTime && iPresentDate>= iStartTime && objInsideMessage.getMessageState() !=null 
							&& objInsideMessage.getMessageState() != 1 ){
						//站内消息状态(0.未发送1.发送中2.已发送)
						objInsideMessage.setMessageState(1);
						//更新站内消息
	 ...
	 *
	 */
	//@Scheduled(cron="0 15 04 * * ?")//每天上午04:15 触发
