			...
			try {
				...
				if (drtFinPoAccountList != null
					...
					for (DrtFinPoAccount drtFinPoAccount : drtFinPoAccountList) {
						...
						if (poEarningsComputingMoney != null && poEarningsComputingMoney.longValue() >= 100*100) {
							// 获取该金额对应的规则
							...
							if (uniquePointsRule != null) {
								...
								double dayIncome = NumberUtils
										.toDouble(uniquePointsRule
												.getDayIncome());
								double coefficient = NumberUtils
										.toDouble(uniquePointsRule
												.getCoefficient());
								double bonusColumn = NumberUtils
										.toDouble(uniquePointsRule
												.getBonusColumn());
								double pointsDatumRate = NumberUtils
										.toDouble(uniquePointsRule
												.getPointsDatumRate());
								double earningsPoints = poEarningsComputingMoney
										.longValue()
										/ 100.0
										* dayIncome
										* bonusColumn
										* pointsDatumRate
										* coefficient;
								long points = Math.round(earningsPoints * 100);
								if (points > 0) {
									// 积分收益记录
									...
									drtFinPoEarningsRecord
											.setPmType(NumberTypeEnum.PLUS
													.getValue());
									drtFinPoEarningsRecord.setEarnings((int)points);
			...
			for (DrtFinPoAccountRecord drtFinPoAccountRecord : finPoAccountRecordList) {
				try{
					if(drtFinPoAccountRecord.getPoDealMoney().longValue() < 0){
						drtFinPoEarningsComputingRecordService.moneyConsumeByPoAccountRecordId(drtFinPoAccountRecord.getPoAccountRecordId());
					}else{
