...
public class DrtAccountService {
    ...
    public void deductExpiryEarnings() throws Exception {
        drtAccountMapper.deductExpiryEarnings();
    }
    
    /**
     * 通过自定义非空字段获取记录集(过期积分大于零)
     * 
     * @param drtAccount 登录账户信息表（主要包括电融通账户、密码）
     * @throws Exception 出错抛出异常
     */
    public List<DrtAccount> selectListEcpiryEarningsPositive(DrtAccount drtAccount) {
        return drtAccountMapper.selectListEcpiryEarningsPositive(drtAccount);
    }
}
