...
public class SftpUtil {  
     ...
     * 文件路径前缀. /ddit-remote 
     */  
	  private static final String PRE_FIX = AppConfig.getProperty("PRE_FIX"); 
     ...
     * */
    @SuppressWarnings("unchecked")
	public  static boolean isExist(String folderName,String fileName,ChannelSftp sftp){
    	boolean result=false;
    	try {
			Vector<LsEntry> vector = sftp.ls(PRE_FIX+folderName);
			if(vector!=null&&vector.size()>0){
				  for (LsEntry en : vector) {
					if(en.getFilename().equals(fileName)){
						result= true;
					}
				}
			}else{
