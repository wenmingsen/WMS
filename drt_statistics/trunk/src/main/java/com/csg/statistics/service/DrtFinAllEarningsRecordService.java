...
public class DrtFinAllEarningsRecordService{
	...
	public int insertAndUpdateTotalPoints(DrtFinAllEarningsRecord drtFinAllEarningsRecord) throws Exception{
		if(drtFinAllEarningsRecord != null && drtFinAllEarningsRecord.getEarnings() != null && drtFinAllEarningsRecord.getEarnings().intValue() != 0){
			...
			if(drtAccount != null && drtAccount.getAccountStatus() != null && Objects.equal(AccountStatusEnum.NORMAL.getValue(), drtAccount.getAccountStatus())){
				...
				if(drtFinPoAccount != null && drtFinPoAccount.getTotalEarnings() != null){
					...
					drtFinEarningsStatistics.setAccountId(drtFinAllEarningsRecord.getAccountId());
					drtFinEarningsStatistics.setDynamicEarnings(drtFinAllEarningsRecord.getEarnings());
					drtFinEarningsStatistics.setUpdateTime(new Timestamp(System.currentTimeMillis()));
					drtFinEarningsStatistics.setTotalEarnings(drtFinAllEarningsRecord.getCurrentEarnings());
					drtFinEarningsStatisticsService.insertOrUpdate(drtFinEarningsStatistics);
		...
		log.info("com.csg.statistics.service.DrtFinAllEarningsRecordService.insertAndUpdateTotalPoints方法回滚：" + drtFinAllEarningsRecord);
		// 手动回滚事务
	 ...
	 * @return 积分收支明细表(当前表)
	 */
	public DrtFinAllEarningsRecordHistory convertToDrtFinAllEarningsRecordHistory(DrtFinAllEarningsRecord drtFinAllEarningsRecord){
		...
		drtFinAllEarningsRecordHistory.setCurrDay(drtFinAllEarningsRecord.getCurrDay());
		
		//2018-03-01 添加积分到期日期字段
		drtFinAllEarningsRecordHistory.setExpiryDay(drtFinAllEarningsRecord.getExpiryDay());
